...
pcl::PointCloud<pcl::PointNormal>::Ptr jit_pcl_normals( pcl::PointCloud<pcl::PointXYZ>::Ptr cloud );
pcl::PolygonMesh jit_pcl_genTriangles( pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals );
...
t_jit_err jit_pcl_matrix_calc(t_jit_pcl *x, void *inputs, void *outputs)
{
	...
	if (x && in_matrix && out_matrix)
    {
        ...
        pcl::PointCloud<pcl::PointNormal>::Ptr cloud_w_normals = jit_pcl_normals( cloud );
        ...
        out_minfo.dim[0] = cloud_w_normals->width;
        out_minfo.dim[1] = cloud_w_normals->height;
        ...
        rowstride = out_minfo.dimstride[1];// >> 2L;
        count = 0;
        for (j = 0; j < out_minfo.dim[0]; j++)
        {
            ...
            for( i = 0; i < out_minfo.dim[1]; i++)
            {
                ...
                {
                    fop[0] = cloud_w_normals->points[count].x;
                    fop[1] = cloud_w_normals->points[count].y;
                    fop[2] = cloud_w_normals->points[count].z;
        ...
        //        pcl::PolygonMesh triangles = jit_pcl_genTriangles( cloud_w_normals );
